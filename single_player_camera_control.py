...
# --- Haupt-App-Klasse ---
class FistDetectorApp(QWidget):
    # Signals are kept but primarily used for logging/debugging now
    status_changed = pyqtSignal(int)
    fist_detected = pyqtSignal(bool)

    def __init__(self):
        super().__init__()
        self.setWindowTitle("Spaceship Control")
        ...
        if self.cap and self.cap.isOpened():
            ...
            print("--- Steuerung ---")
            print("Faust im Bild: Simuliert 'a' Taste gedrückt")
            print("Keine Faust / Faust loslassen: Simuliert Taste loslassen")
            print("-----------------")
        else:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if is_fist(hand_landmarks):
                    fist_detected_in_frame = True
                    cv2.putText(frame_bgr, 'GO!', (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
                else:
                    cv2.putText(frame_bgr, 'Turn!', (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
        else:
            cv2.putText(frame_bgr, 'Turn!', (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)

        # --- Keyboard Simulation Logic ---
        if fist_detected_in_frame and not self.key_a_pressed:
            try:
                self.keyboard_controller.press('a')
                self.key_a_pressed = True
                print("SIM: 'a' pressed")
                self.fist_detected.emit(True)
            except Exception as e:
                 print(f"Error pressing 'a': {e}")
        elif not fist_detected_in_frame and self.key_a_pressed:
            try:
                self.keyboard_controller.release('a')
                self.key_a_pressed = False
                print("SIM: 'a' released")
                self.fist_detected.emit(False)
            except Exception as e:
                 print(f"Error releasing 'a': {e}")

        # Update overall status signal
        now = time.time()
        if fist_detected_in_frame:
